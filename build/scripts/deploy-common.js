...
const COMMON_JSON = JSON.parse(
  fs.readFileSync(path.join(REPO_ROOT, "build", "common.json"), "utf8"),
);
...
if (fs.existsSync(THEME_CONFIG)) {
  try {
    ...
    console.warn(
      `deploy-common: unable to read theme config ${THEME_CONFIG}: ${error.message}`,
    );
  }
}

const APP_TITLE_TEXT =
  process.env.APP_TITLE_TEXT || themeMeta.app_title || "Euro Office";
...
function deploySDK() {
  ...
  // cell: css and sdk-*.js
  copyDirFiltered(
    path.join(SDKJS_ROOT, "cell", "css"),
    path.join(sdkOut, "cell", "css"),
    { include: ["*.css"] },
  );
  ...
  copyDir(
    path.join(SDKJS_ROOT, "slide", "themes"),
    path.join(sdkOut, "slide", "themes"),
  );
...
(async () => {
  ...
  for (const v of VENDORS.filter((v) => !v.dir && !v.minify)) {
    ...
    copyFile(path.join(VENDOR_SRC, v.src), path.join(BUILD_OUT, v.dest), {
      required: true,
    });
